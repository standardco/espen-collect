package org.odk.collect.android.database.lookups;
...
import static org.odk.collect.android.database.DatabaseConstants.LOOKUP_DATABASE_NAME;
import static org.odk.collect.android.database.DatabaseConstants.LOOKUP_TABLE_NAME;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.INSTANCE_PATH;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_1;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_2;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_3;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_4;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_5;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_6;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_7;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_8;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_9 ;
import static org.odk.collect.android.database.lookups.DatabaseLookupColumns.COLUMN_10;
...
public class LookupDatabaseMigrator implements DatabaseMigrator {
    ...
    public void onCreate(SQLiteDatabase db) {
        createInstancesTableV5(db, LOOKUP_DATABASE_NAME);
...
//        dropObsoleteColumns(db, CURRENT_VERSION_COLUMN_NAMES, temporaryTableName);
    ...
    private void upgradeToVersion5(SQLiteDatabase db) {
        String temporaryTableName = LOOKUP_TABLE_NAME;
    ...
    private void dropObsoleteColumns(SQLiteDatabase db, String[] relevantColumns, String temporaryTableName) {
        List<String> columns = SQLiteUtils.getColumnNames(db, LOOKUP_TABLE_NAME);
        columns.retainAll(Arrays.asList(relevantColumns));
        String[] columnsToKeep = columns.toArray(new String[0]);

        SQLiteUtils.copyRows(db, LOOKUP_TABLE_NAME, columnsToKeep, temporaryTableName);
        SQLiteUtils.dropTable(db, LOOKUP_TABLE_NAME);
        SQLiteUtils.renameTable(db, temporaryTableName, LOOKUP_TABLE_NAME);
    }
...
//                + DELETED_DATE + " date );");
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS " + LOOKUP_TABLE_NAME + " ("
                + _ID + " integer primary key, "
