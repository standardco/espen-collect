...
class ProjectResetter(
    ...
    private fun resetPreferences() {
        org.espen.collect.android.utilities.WebCredentialsUtils.clearAllCredentials()
        ...
        if (!deleteFolderContent(storagePathProvider.getOdkDirPath(org.espen.collect.android.storage.StorageSubdirectory.SETTINGS))) {
    ...
    private fun resetInstances() {
        ...
        if (!deleteFolderContent(storagePathProvider.getOdkDirPath(org.espen.collect.android.storage.StorageSubdirectory.INSTANCES))) {
    ...
    private fun resetForms() {
        ...
        File(storagePathProvider.getOdkDirPath(org.espen.collect.android.storage.StorageSubdirectory.METADATA) + File.separator + org.espen.collect.android.fastexternalitemset.ItemsetDbAdapter.DATABASE_NAME).delete()

        if (!deleteFolderContent(storagePathProvider.getOdkDirPath(org.espen.collect.android.storage.StorageSubdirectory.FORMS))) {
            failedResetActions.add(ResetAction.RESET_FORMS)
        }
    }

    private fun resetLayers() {
        if (!deleteFolderContent(storagePathProvider.getOdkDirPath(org.espen.collect.android.storage.StorageSubdirectory.LAYERS))) {
            failedResetActions.add(ResetAction.RESET_LAYERS)
        }
    }

    private fun resetCache() {
        if (!deleteFolderContent(storagePathProvider.getOdkDirPath(org.espen.collect.android.storage.StorageSubdirectory.CACHE))) {
